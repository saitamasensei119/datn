...
const Departments: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn xóa khoa này? Hành động này có thể ảnh hưởng đến các lớp học phần và giảng viên liên quan.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Không thể xóa khoa. Khoa có thể đang chứa giảng viên hoặc môn học.",
      );
