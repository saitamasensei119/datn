...
const Semesters: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Bạn có chắc muốn xóa học kỳ này? Các lớp học phần đăng ký cho học kỳ này sẽ bị xóa theo.')) {
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Không thể xóa học kỳ. Học kỳ có thể đang chứa lớp học phần hoạt động.');
  ...
  return (
    <div className="glass-card">
      <div className="page-header-flex">
        <div>
          <h2 className="page-title" style={{ margin: 0 }}>Quản Lý Học Kỳ</h2>
          ...
          <table className="custom-table">
            ...
            <tbody>
              {semesters.map((sem) => (
                <tr key={sem.id}>
                  <td>{sem.id}</td>
                  <td style={{ fontWeight: '600', color: 'var(--primary)' }}>{sem.name}</td>
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">
                {modalType === 'create' ? 'Tạo học kỳ mới' : 'Cập nhật học kỳ'}
              </h3>
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="sem-name">
                    Tên học kỳ <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="sem-name"
                    type="text"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="end-date">
                    Ngày kết thúc <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="end-date"
                    type="date"
