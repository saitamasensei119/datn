...
import { Plus, Edit2, Trash2, X, AlertTriangle, BookMarked } from 'lucide-react';
...
const Subjects: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedSubjects = subjectRes.data.map((subj: any) => ({
        ...
        departmentName: subj.department ? subj.department.name : 'Chưa phân khoa',
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa môn học này? Các lớp học phần liên quan đến môn học này sẽ bị ảnh hưởng.')) {
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Không thể xóa môn học. Môn học này có thể đang được giảng dạy trong một lớp học phần.');
  ...
  return (
    <div className="glass-card">
      <div className="page-header-flex">
        <div>
          <h2 className="page-title" style={{ margin: 0 }}>Quản Lý Môn Học</h2>
          ...
          <table className="custom-table">
            <thead>
              <tr>
                ...
                <th>Tên môn học</th>
                <th style={{ width: '100px', textAlign: 'center' }}>Số tín chỉ</th>
              ...
              {subjects.map((subj) => (
                <tr key={subj.id}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <span className="badge badge-info">{subj.credits} TC</span>
                  </td>
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">
                {modalType === 'create' ? 'Tạo môn học mới' : 'Cập nhật môn học'}
              </h3>
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="subj-dept">
                    Khoa phụ trách quản lý <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  {departments.length === 0 ? (
                    ...
                    <select
                      ...
                      required
                    >
                      <option value="" disabled>-- Chọn khoa phụ trách --</option>
