...
const Lecturers: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedLecturers = lectRes.data.map((l: any) => ({
        ...
        departmentId: deptRes.data.find((d: any) => d.name === l.departmentName)?.id || undefined
      }));
  ...
  const mappedSubjects = (lects: any[], depts: Department[]) => {
    return lects.map(l => {
      if (!l.departmentId && l.departmentName) {
        const found = depts.find(d => d.name.toLowerCase() === l.departmentName.toLowerCase());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !email.trim() || !lecturerCode.trim() || !departmentId) {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Có lỗi xảy ra khi lưu giảng viên.');
    }
  };

  const handleDelete = async (id: number) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa giảng viên này? Hệ thống cũng sẽ xóa tài khoản người dùng tương ứng.')) {
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Không thể xóa giảng viên này. Giảng viên đang được phân công dạy ở các lớp học phần.');
  ...
  return (
    <div className="glass-card">
      <div className="page-header-flex">
        <div>
          <h2 className="page-title" style={{ margin: 0 }}>Quản Lý Giảng Viên</h2>
        ...
        <div className="table-container">
          <table className="custom-table">
            ...
            <tbody>
              {lecturers.map((lect) => (
                <tr key={lect.id}>
                  <td style={{ fontWeight: '600', color: 'var(--primary)' }}>{lect.lecturerCode}</td>
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">
                {modalType === 'create' ? 'Thêm giảng viên mới' : 'Cập nhật thông tin'}
              </h3>
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="lect-code">
                    Mã giảng viên <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="lect-code"
                    type="text"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="lect-name">
                    Họ và tên giảng viên <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="lect-name"
                    type="text"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="lect-dept">
                    Khoa / Ngành công tác <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  {departments.length === 0 ? (
                    ...
                    <select
                      ...
                      required
                    >
                      <option value="" disabled>-- Chọn khoa công tác --</option>
