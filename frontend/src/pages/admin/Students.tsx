...
import { Plus, Edit2, Search, X, AlertTriangle, User, GraduationCap, CheckCircle } from 'lucide-react';
...
const Students: React.FC = () => {
  ...
  const [searchedStudent, setSearchedStudent] = useState<StudentProfile | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !email.trim() || !studentCode.trim() || !departmentId) {
    ...
    try {
      if (modalType === 'create') {
        ...
        setSuccess(`Thêm sinh viên thành công! Sinh viên được tạo có ID: ${response.data.id}`);
        ...
        setSearchedStudent({
          ...
          studentCode,
          departmentId: Number(departmentId),
          departmentName: departments.find(d => d.id === Number(departmentId))?.name || 'Chưa có'
        });
      }
      setIsModalOpen(false);
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Có lỗi xảy ra khi lưu dữ liệu sinh viên.');
    }
  };
  ...
  return (
    <div className="glass-card">
      <div className="page-header-flex">
        <div>
          <h2 className="page-title" style={{ margin: 0 }}>Quản Lý Sinh Viên</h2>
      ...
      <div style={{ 
        ...
        💡 <strong>Thông tin hệ thống:</strong> Vì các tính năng lấy danh sách toàn bộ sinh viên (`getAll`) và xóa sinh viên (`delete`) hiện chưa được mở trên máy chủ backend của bạn, bạn có thể thực hiện <strong>Thêm sinh viên mới</strong> (nút ở trên) hoặc <strong>Tra cứu sinh viên cụ thể theo mã ID</strong> bằng hộp tìm kiếm bên dưới.
      </div>

      <div className="details-grid" style={{ gridTemplateColumns: '1fr', maxWidth: '600px', margin: '0 auto 2rem auto' }}>
        {/* Search Form Card */}
        <div className="glass-card" style={{ padding: '1.5rem', marginBottom: 0 }}>
          <h3 style={{ fontSize: '1rem', marginBottom: '1rem', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Search size={18} style={{ color: 'var(--primary)' }} />
            <span>Tra cứu Sinh viên bằng ID</span>
          </h3>
          <form onSubmit={handleSearch} className="search-bar-container" style={{ maxWidth: 'none' }}>
            <input
              type="number"
          ...
          {searchError && (
            <div className="alert alert-danger" style={{ marginTop: '1rem', textAlign: 'left', marginBottom: 0 }}>
      ...
      {searchedStudent && (
        <div className="glass-card" style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'left', borderLeft: '4px solid var(--primary)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
            <div className="detail-item">
              ...
              <div className="detail-value" style={{ color: 'var(--primary)' }}>{searchedStudent.studentCode}</div>
            </div>
            ...
            <div className="detail-item">
              ...
              <div className="detail-value">{searchedStudent.departmentName}</div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">
                {modalType === 'create' ? 'Tạo sinh viên mới' : 'Cập nhật sinh viên'}
              </h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="std-code">
                    Mã số sinh viên <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="std-code"
                    type="text"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="std-email">
                    Email đăng nhập <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input
                    id="std-email"
                    type="email"
                ...
                {modalType === 'create' && (
                  <div className="form-group">
                    <label className="form-label" htmlFor="std-password">
                      Mật khẩu tài khoản <span style={{ color: 'var(--danger)' }}>*</span>
                    </label>
                    <input
                      id="std-password"
                      type="password"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="std-dept">
                    Khoa đào tạo quản lý <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  {departments.length === 0 ? (
                    ...
                    <select
                      ...
                      required
                    >
                      <option value="" disabled>-- Chọn khoa đào tạo --</option>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={departments.length === 0}>
